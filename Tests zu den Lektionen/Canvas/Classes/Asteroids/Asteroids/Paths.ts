namespace L09_Asteroids {
    ...
    export function createPaths(): void { // liest Koordinaten aus und erzeugt 3D Pfade 
        asteroidPaths = createAsteroidPaths(shapesAsteroids);
        ufoPath = createUfoPath();
    }

    function createAsteroidPaths(_shapes: number[][][]): Path2D[] { // Die Pfade kann man dann zum Zeichnen verwenden.
        let paths: Path2D[] = [];
