namespace generativeArt2 {
    ...
    function draw(): void {
        ...
        let hex: string[] = ["#247BA0", "#70C1B3", "#B2DBBF", "#F3FFBD", "#FF1654", "#FDFFFC", "#2EC4B6", "#FF9F1C", "#E71D36", "#E63946", "#F1FAEE", "#A8DADC", "#457B9D", "#1D3557", "#2B2D42", "#8D99AE", "#EDF2F4", "#EF233C", "#D90429", "#6699CC", "#FFF275", "#FF8C42", "#FF3C38", "#A23E48", "#1A535C", "#4ECDC4", "#F7FFF7", "#FF6B6B", "#FFE66D", "#06AED5", "#086788", "#F0C808", "#FFF1D0", "#DD1C1A"];

        // Canvas
        crc2.fillStyle = "black";
        crc2.fillRect(0, 0, canvas.width, canvas.height);

        let prevX: number = 100;
        let prevY: number = 50;
        let points: number = Math.floor(Math.random() * 100 + 20);
        crc2.beginPath();

        for (let i: number = 0; i < points; i++) {

            let x: number = Math.floor(Math.random() * 800); // Für gesamte Breite
            let y: number = Math.floor(Math.random() * 800); // Für gesamte Höhe
            let color: string = hex[Math.floor(Math.random() * hex.length)]; // Zufällige Farbe aus dem hex-string
        ...
        // Linie ausführen
        function drawLine(x: number, y: number, _color: string): void {
